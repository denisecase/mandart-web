...
export default class ColorEditorRow {
  ...
  _setupDragEvents(row) {
    ...
    row.addEventListener('drop', (e) => {
      ...
      console.log("DROP EVENT FIRED on row", this.index);
      ...
      try {
        ...
        console.log(`🔄 Dropping color from position ${fromIndex + 1} to position ${toIndex + 1}`);
        ...
        // Call the reordering callback
        if (this.reorderCallback) {
          console.log("Calling reorderCallback with", fromIndex, toIndex);
          this.reorderCallback(fromIndex, toIndex);
        } else {
          console.error("reorderCallback is not defined!");
        }
      } catch (error) {
