...
class ArtImage {
  ...
  complexPow(baseX, baseY, powerReal) {
    if (this.isMandArt()) {
      ...
      return [
        (xSquared - 3.0 * ySquared) * baseX,
        (3.0 * xSquared - ySquared) * baseY,
      ];
    } else {
  ...
  generateGrid() {
    ...
    let fIter = Array.from({ length: imageWidth }, () =>
      Array(imageHeight).fill(0)
    );
  ...
  renderMandelbrot(canvas) {
    ...
    let imgData = ctx.createImageData(
      this.shapeInputs.imageWidth,
      this.shapeInputs.imageHeight
    );
