...
function saveMandelbrotAsPng() {
    ...
    if (fileSelect && fileSelect.selectedOptions && fileSelect.selectedOptions[0]) {
        // Get the actual text of the selected option, not just its value
        const selectedOption = fileSelect.selectedOptions[0];
        const selectedText = selectedOption.textContent || selectedOption.innerText;
        
        // Use the displayed text if available, otherwise fall back to value
        selectedName = selectedText || fileSelect.value;
        
        // Remove file extension and path if present
        selectedName = selectedName.split('/').pop(); // Remove any path
        selectedName = selectedName.replace(/\.[^/.]+$/, ""); // Remove extension
        
        console.log(`Using selected name as prefix: ${selectedName}`);
    } else if (canvas.title && canvas.title.trim() !== "") {
        // If no dropdown value, try the canvas title
        selectedName = canvas.title.replace(/\.[^/.]+$/, "");
    }
